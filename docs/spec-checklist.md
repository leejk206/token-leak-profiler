...
**Enforced by:** `tests/test_rules_self_application.py` (parametrized across all registered analyzers).

## Rule 6: leaked_tokens가 측정값인가 모델 출력인가 (v0.6.1 추가)

새 분석기 spec 작성 시:

> "leaked_tokens가 사용자 transcript에서 직접 추출된 측정값인가, 아니면 임계값·상수·heuristic이 곱해진 모델 출력인가?"

- 모델 출력 (e.g., `count × estimated_tokens_per_X`) → **`evidence_kind="estimated"` 강제**, 신뢰구간을 evidence dict에 명시.
- 측정값 + 처방 1:1 → `confirmed`.
- 측정값 + 처방 미확정 → `signal`.

룰 6은 룰 5의 정밀화. 룰 5는 "처방 1:1 검증"만 본다 — 처방이 명확해도 측정값 자체가 추정이면 confirmed 부적합.

**Enforced by:** `tests/test_rules_self_application.py` (parametrized across all registered analyzers).

**과거 사례:**
- v0.6.0 mcp_server_overhead: `activated_count × 200`을 confirmed로 분류 → 룰 6 위반 → v0.6.1 fix로 estimated로 강등.

**룰 6 자기-적용 일관성:** 기존 lever 재평가:
- format_boilerplate: 측정값 (반복 횟수 × 평균 토큰)이 모두 transcript 기반 — confirmed 유지
- cache_turnover.recoverable: 측정값 (실제 cache_creation) — confirmed 유지
- redundant_restatement: 측정값 (실제 토큰 수) — confirmed 유지
- subagent_context_overdump: 측정값 (실제 first prompt 토큰) — confirmed 유지
- mcp_server_overhead: 추정값 (200 tok/tool 곱셈) — estimated (v0.6.1 fix)
